...
func (p *PluginState) Handler4(req, resp *dhcpv4.DHCPv4) (*dhcpv4.DHCPv4, bool) {
	...
	case dhcpv4.MessageTypeRequest:
		...
		if err := p.leaseIP(ctx, req.ClientHWAddr, req.RequestedIPAddress(),
			resp.IPAddressLeaseTime(30*time.Second)); err != nil {
			log.Errorf("unable to lease nic %s, ip %s: %w", req.ClientHWAddr, req.RequestedIPAddress(), err)
			return nil, true
		}

		// peek first from list of free ips
		resp.YourIPAddr = req.RequestedIPAddress()

		log.Infof("return requested IP %s for MAC %s", req.RequestedIPAddress(), req.ClientHWAddr)
